...
class FP32_Adder(parameters: Map[String, Any]) extends Module with Convert {
    ...
    val stages = 2 + (if(config("input_buffer")) 1 else 0) + (if(config("output_buffer")) 1 else 0)
    if(config("valid")) io.valid_out.get := ShiftRegister(io.valid_in.get, stages - 1)
...
class FP32_Mult(parameters: Map[String, Any]) extends Module with Convert {
    ...
    val ze = RegNext(Cat(0.U(2.W), xe) + ye - Mux(carry, 126.U, 127.U))
