...
class IfmBuffer extends Module with mesh_config with buffer_config {
  ...
  // ################ def addr 1 ################
  val ow_cnt1            = RegInit(0.U(12.W))
  val oh_cnt1            = RegInit(0.U(12.W))
  val ow_cnt1_block_row0 = RegInit(0.U(12.W))
  val oh_cnt1_block_row0 = RegInit(0.U(12.W))
  ...
  val ow_cnt1_initial     = ow_cnt1_initial_lut(Mux(io.ofm_size.w >= 32.U, 31.U, io.ofm_size.w - 1.U))
  val ow_cnt1_block_row0_temp = Mux(
    (ow_cnt1 + 33.U) > io.ofm_size.w - 1.U,
    MuxCase(
      (ow_cnt1 + 33.U - io.ofm_size.w),
      Array(
        (io.ofm_size.w < 32.U) -> ((ow_cnt1 + 33.U) % io.ofm_size.w(4, 0)),
        (io.ofm_size.w === 32.U) -> 0.U
      )
    ),
    ow_cnt1 + 33.U
  )
  ...
  when(riseEdge(io.task_done)) {
    ow_cnt1            := ow_cnt1_initial
    ow_cnt0_block_row0 := ow_cnt1_initial
  }.elsewhen(io.ifm.ready) {
    ow_cnt1 := ow_cnt1 + 1.U
    when(last_clk_of_one_block) {
      ow_cnt1 := ow_cnt1_block_row0
      when(this_ofm_last_block) {
        ow_cnt1_block_row0 := ow_cnt1_block_row0_temp
        ow_cnt1            := ow_cnt1_block_row0_temp
      }
  ...
  val oh_cnt1_block_row0_temp = Mux(
    (ow_cnt1 + 33.U) > io.ofm_size.w - 1.U,
    MuxCase(
      (oh_cnt1 + 1.U),
      Array(
        (io.ofm_size.w < 32.U) -> (oh_cnt1 + (ow_cnt1 + 33.U).asUInt / io.ofm_size.w(4, 0)),
        (io.ofm_size.w === 32.U) -> (oh_cnt1 + 2.U)
      )
    ),
    oh_cnt1
  )
  when(riseEdge(io.task_done)) {
    ...
    when(last_clk_of_one_block) {
      ...
      when(this_ofm_last_block) {
        oh_cnt1_block_row0 := oh_cnt1_block_row0_temp
        oh_cnt1            := oh_cnt1_block_row0_temp
      }
    }.elsewhen(ow_cnt1 === io.ofm_size.w - 1.U) {
      oh_cnt1 := oh_cnt1 + 1.U
    }
